...
A `js` node has exactly ONE output and CANNOT branch — never give a `js` node `handlers`, and never draw an edge with a `port` from it. Branching is always a `rule` node, and each handler `name` is a routed port. If you were about to put `handlers` on a `js` node, make it a `rule` node instead.
A Rule node's `logic_rule` must return ONE STRING equal to a handler's `name`. That string is the port that fires (e.g. `let decision = amount < 100 ? 'approve' : 'review'; decision`). To fire several ports at once, return an array of handler names (`['notify', 'archive']`). Returning anything else — a string that matches no handler, `''`, `null`, `undefined`, a number, an OBJECT like `{ approve: true }` — fires no tag, and a Rule that fires no tag prunes every outgoing edge, so that branch of the flow ends silently. There is no error and no log line. So make the returned value exhaustive over the declared handlers: write `let decision = ok ? 'ok' : 'retry'`, NOT `let decision; if (ok) { decision = 'ok' }` — the second form leaves `decision` undefined in the else case and produces a dead port that looks fine on the canvas and fails invisibly at run time.
